...
The system is operated from MENU. A user can perform 7 operation(s): MENU start-up (login / session setup); Option '1': SALDO; Option '2': SAQUE; Option '3': DEPOSITO; Option '4': TRANSFERENCIA; Option '5': EXTRATO; Option '9': Set WS-FIM to 'S'. Across these operations the code applies 11 validation check(s) and makes 7 write(s) to stored data in: CLI-REG, CLIENTES, MOV-REG, MOVTOS. Process names below are taken from the code's own labels; their business meaning (what a routine called SAQUE is *for*) is a reading of the code, not something the code states. Each step in the tables cites the line it comes from.
...
|---|---|---|---|---|---|
| 1 | MENU | ⌨️ Take input WS-CONTA (prompt "CONTA:") |  |  | `MENU.cbl:43` |
| 2 | MENU | 💾 READ CLIENTES |  |  | `MENU.cbl:45` |
| 3 | MENU | ⚠️ Show "CONTA INVALIDA" |  |  | `MENU.cbl:47` |
| 4 | MENU | ❓ Check `CLI-STATUS = 'B'` | show "CONTA BLOQUEADA" | continue | `MENU.cbl:50-51` |
| 5 | MENU | → Call KBDREAD |  |  | `MENU.cbl:56` |
| 6 | MENU | ❓ Check `WS-SENHA NOT = CLI-SENHA` | show "SENHA INVALIDA" | continue | `MENU.cbl:57-59` |
| 7 | MENU | ❓ Check `WS-TENTATIVAS >= 3` | show "CARTAO BLOQUEADO" and abort the operation | continue | `MENU.cbl:31` |
...
| # | Unit | Step | If true | If false | Evidence |
|---|---|---|---|---|---|
| 1 | MENU | → Call CONTA with operation 'S' |  |  | `MENU.cbl:67` |
| 2 | CONTA | → CONTA dispatches 'S' to SALDO |  |  | `CONTA.cbl:47-48` |
| 3 | CONTA | → Call UTIL with operation 'F' |  |  | `CONTA.cbl:59` |
| 4 | UTIL | → UTIL dispatches 'F' to FORMATA |  |  | `UTIL.cbl:14-15` |
...
| 4 | CONTA | ❓ Check `WS-VALOR > WS-LIMITE-SAQUE` | show "LIMITE DE SAQUE EXCEDIDO" and abort the operation | continue | `CONTA.cbl:64-65` |
| 5 | CONTA | ❓ Check `WS-VALOR > LK-SALDO` | show "SALDO INSUFICIENTE" and abort the operation | continue | `CONTA.cbl:68-69` |
| 6 | CONTA | ❓ Check `FUNCTION MOD(WS-VALOR, 10) NOT = 0` | show "VALOR DEVE SER MULTIPLO DE 10" and abort the operation | continue | `CONTA.cbl:72-73` |
| 7 | CONTA | → Perform GRAVA |  |  | `CONTA.cbl:78` |
...
| # | Unit | Step | If true | If false | Evidence |
|---|---|---|---|---|---|
| 1 | MENU | → Call CONTA with operation 'D' |  |  | `MENU.cbl:71` |
| 2 | CONTA | → CONTA dispatches 'D' to DEPOSITO |  |  | `CONTA.cbl:51-52` |
| 3 | CONTA | ⌨️ Take input WS-VALOR (prompt "VALOR DO DEPOSITO:") |  |  | `CONTA.cbl:81` |
| 4 | CONTA | ❓ Check `WS-VALOR = 0` | show "VALOR INVALIDO" and abort the operation | continue | `CONTA.cbl:82-83` |
| 5 | CONTA | → Perform GRAVA |  |  | `CONTA.cbl:88` |
| 6 | CONTA | 💾 REWRITE CLI-REG |  |  | `CONTA.cbl:117` |
| 7 | CONTA | 💾 WRITE MOV-REG |  |  | `CONTA.cbl:121` |
...
| # | Unit | Step | If true | If false | Evidence |
|---|---|---|---|---|---|
| 1 | MENU | → Call CONTA with operation 'T' |  |  | `MENU.cbl:73` |
| 2 | CONTA | → CONTA dispatches 'T' to TRANSFERENCIA |  |  | `CONTA.cbl:53-54` |
| 3 | CONTA | ⌨️ Take input WS-DESTINO (prompt "CONTA DESTINO:") |  |  | `CONTA.cbl:91` |
| 4 | CONTA | ⌨️ Take input WS-VALOR (prompt "VALOR:") |  |  | `CONTA.cbl:93` |
| 5 | CONTA | ❓ Check `WS-VALOR > LK-SALDO` | show "SALDO INSUFICIENTE" and abort the operation | continue | `CONTA.cbl:95-96` |
| 6 | CONTA | 💾 READ CLIENTES |  |  | `CONTA.cbl:100` |
| 7 | CONTA | ⚠️ Show "CONTA DESTINO INEXISTENTE" |  |  | `CONTA.cbl:102` |
| 8 | CONTA | ❓ Check `CLI-STATUS = 'B'` | show "CONTA DESTINO BLOQUEADA" and abort the operation | continue | `CONTA.cbl:105-106` |
| 9 | CONTA | 💾 REWRITE CLI-REG |  |  | `CONTA.cbl:111` |
| 10 | CONTA | → Perform GRAVA |  |  | `CONTA.cbl:114` |
| 11 | CONTA | 💾 REWRITE CLI-REG |  |  | `CONTA.cbl:117` |
| 12 | CONTA | 💾 WRITE MOV-REG |  |  | `CONTA.cbl:121` |
...
| # | Unit | Step | If true | If false | Evidence |
|---|---|---|---|---|---|
| 1 | MENU | → Call EXTRATO |  |  | `MENU.cbl:75` |
| 2 | EXTRATO | 🖥️ Display "--- EXTRATO ---" |  |  | `EXTRATO.cbl:27` |
| 3 | EXTRATO | 💾 READ MOVTOS |  |  | `EXTRATO.cbl:29` |
| 4 | EXTRATO | → Perform MOSTRA |  |  | `EXTRATO.cbl:31` |
| 5 | EXTRATO | ❓ Check `MOV-CONTA = LK-CONTA` | take the guarded branch | continue | `EXTRATO.cbl:40` |
| 6 | EXTRATO | → Call UTIL with operation 'F' |  |  | `EXTRATO.cbl:42` |
| 7 | UTIL | → UTIL dispatches 'F' to FORMATA |  |  | `UTIL.cbl:14-15` |
| 8 | EXTRATO | 🖥️ Display "MOV-TIPO" |  |  | `EXTRATO.cbl:43` |
| 9 | EXTRATO | ❓ Check `WS-QTD = 0` | show "SEM MOVIMENTOS" | continue | `EXTRATO.cbl:34-35` |
...
| # | Unit | Step | If true | If false | Evidence |
|---|---|---|---|---|---|
| 1 | MENU | 🖥️ Set WS-FIM to 'S' |  |  | `MENU.cbl:77` |
